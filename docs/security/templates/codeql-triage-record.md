...
| 26     | Medium   | Local API OpenAPI JSON written to the tracked contract artifact                      | Default source is fixed loopback `/v1/docs-json`; response status checked, JSON parsed/canonicalized, destination is a fixed module-relative path; generation is an explicit developer command | False positive: controlled local contract generation                        | Not approved               |
...
- [ ] Alert 26 loopback default, fixed destination and explicit generation boundary were verified.
