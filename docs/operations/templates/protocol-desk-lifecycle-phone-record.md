# Protocol Desk staging lifecycle and Principal-phone record
...
Use only synthetic requester and engagement data in production-like staging.
The Principal must personally operate their own recorded phone; desktop browser
automation, emulation, a proxy operator or a generic device-lab Desk result is
supporting evidence only. Retain durable provider, HTTP, audit and device-test
references without copying mailbox addresses, request prose, Protocol Note
contents, capability tokens, cookies, credentials or personal data into this
repository.
