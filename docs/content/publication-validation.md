...
- Every author-supplied public URL uses HTTPS. Identity `sameAs`, Archive media, Source Register links and Protocol Desk organisation websites reject executable, embedded, legacy-transfer and plaintext schemes before persistence or projection; Cloudinary upload/delivery contracts apply the same transport rule.
- Every JSON-LD graph uses the shared inline-script serializer. CMS-authored and route-derived values preserve their JSON meaning while HTML tag openers are escaped, preventing a value from terminating the script element and creating executable markup.
