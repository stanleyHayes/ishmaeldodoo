...
function liteNoScriptPolicy(): string {
  return [
    "default-src 'self'",
    "base-uri 'self'",
    "object-src 'none'",
...
export function proxy(request: NextRequest) {
  ...
  const queryLite =
    isLiteValue(request.nextUrl.searchParams.get("lite")) ||
    isLiteValue(request.nextUrl.searchParams.get("mode"));
  const queryCameFromNavigation =
    request.headers.get("rsc") !== "1" &&
    request.headers.get("next-router-prefetch") !== "1";
  const storedLite = request.cookies.get(liteCookieName)?.value;
  const liteDismissed = storedLite === "dismissed";
  const liteActive = queryLite || (!liteDismissed && isLiteValue(storedLite));
  ...
  if (
    ...
    if (queryLite && queryCameFromNavigation) {
      response.cookies.set(liteCookieName, "1", {
        httpOnly: false,
  ...
  requestHeaders.set("x-amanor-lite", liteActive ? "1" : "0");
  ...
  if (liteActive && ["/", "/fr", "/press", "/fr/press"].includes(pathname)) {
    response.headers.set("Content-Security-Policy", liteNoScriptPolicy());
  }
  if (queryLite && queryCameFromNavigation) {
    response.cookies.set(liteCookieName, "1", {
      httpOnly: false,
